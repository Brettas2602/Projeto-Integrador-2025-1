...
export default function LoginEnfermeiro() {
    ...
    const { enfermeiro, setEnfermeiro } = useUser()

    const router = useRouter()

    useEffect(() => {
        if (enfermeiro) router.replace("dashboard")
    }, [])

    async function checkUserData() {
        try {
            const { data } = await axios.get(`http://localhost:8000/enfermeiro/${cpf}`)
        ...
        <div className="max-w-screen-md mx-auto w-full h-screen flex justify-center items-center px-2">
            ...
            {message && (
                ...
                <div className="flex w-full justify-around items-center">
                    ...
                    <Link href="/cadastroEnfermeiro" className="border-b-2 border-[#FFB8B8] text-center pb-2" >
                        <p>Cadastre-se</p>
                    </Link>
                </div>
